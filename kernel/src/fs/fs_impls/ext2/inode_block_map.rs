...
impl BlockPtrTree {
    ...
    ///
    fn walk_block_chain(&self, path: &BlockPointerPath, fs: &Ext2) -> Result<BranchChainWalkResult> {
    ...
    ///
    pub(super) fn lookup_or_alloc_block(
        &mut self,
        fs: &Ext2,
        iblock: u32,
        create: bool,
        ...
        Ok(self
            .lookup_or_alloc_block_range(fs, iblock, 1, create)?
            .map(|range| range.start))
    }
