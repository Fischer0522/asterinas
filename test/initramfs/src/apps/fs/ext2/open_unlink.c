...
#define FILE_UNLINK BASE_DIR "/unlink_file"
...
FN_TEST(open_unlink_write_read)
{
	remove_file_if_exists(FILE_UNLINK);

	int fd = TEST_SUCC(open(FILE_UNLINK, O_CREAT | O_RDWR, 0644));

	TEST_SUCC(unlink(FILE_UNLINK));

	const char *msg = "hello";
	TEST_RES(write(fd, msg, 5), _ret == 5);

	TEST_SUCC(lseek(fd, 0, SEEK_SET));

	char buf[16] = { 0 };
	TEST_RES(read(fd, buf, sizeof(buf)), _ret == 5);
	TEST_RES(memcmp(buf, "hello", 5), _ret == 0);

	struct stat st;
	TEST_ERRNO(stat(FILE_UNLINK, &st), ENOENT);
