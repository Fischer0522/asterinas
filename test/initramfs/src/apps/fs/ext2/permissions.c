...
FN_TEST(umask_affects_creat)
{
	// umask returns the previous mask; it cannot fail
	mode_t old_mask = umask(0077);

	int fd = TEST_SUCC(creat(TEST_FILE, 0666));
	TEST_SUCC(close(fd));

	struct stat st;
	TEST_RES(stat(TEST_FILE, &st), (st.st_mode & 0777) == 0600);
