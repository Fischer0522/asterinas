// SPDX-License-Identifier: MPL-2.0

#include <errno.h>
#include <fcntl.h>
#include <stdio.h>
#include <string.h>
#include <sys/stat.h>
#include <sys/xattr.h>
#include <unistd.h>

#include "../../common/test.h"

#define BASE_DIR "/ext2/xattr_test"

static void ensure_base_dir(void)
{
	CHECK_WITH(mkdir(BASE_DIR, 0755), _ret >= 0 || errno == EEXIST);
}

FN_SETUP(prepare_base_dir)
{
	ensure_base_dir();
}
END_SETUP()

FN_TEST(xattr_set_get_roundtrip)
{
	const char *path = BASE_DIR "/roundtrip";
	int fd = CHECK(open(path, O_CREAT | O_WRONLY, 0644));
	CHECK(close(fd));

	TEST_SUCC(setxattr(path, "user.test", "value123", 8, 0));

	char buf[64] = { 0 };
	TEST_RES(getxattr(path, "user.test", buf, sizeof(buf)), _ret == 8);
	TEST_RES(strcmp(buf, "value123"), _ret == 0);

	CHECK(unlink(path));
}
END_TEST()

FN_TEST(xattr_list)
{
	const char *path = BASE_DIR "/listfile";
	int fd = CHECK(open(path, O_CREAT | O_WRONLY, 0644));
	CHECK(close(fd));

	TEST_SUCC(setxattr(path, "user.alpha", "a", 1, 0));
	TEST_SUCC(setxattr(path, "user.beta", "b", 1, 0));

	char buf[256] = { 0 };
	ssize_t len =
		TEST_RES(listxattr(path, buf, sizeof(buf)), _ret > 0);

	if (len > 0) {
		int found_alpha = 0, found_beta = 0;
		for (const char *p = buf; p < buf + len;
		     p += strlen(p) + 1) {
			if (strcmp(p, "user.alpha") == 0)
				found_alpha = 1;
			if (strcmp(p, "user.beta") == 0)
				found_beta = 1;
		}
		TEST_RES(found_alpha && found_beta, _ret != 0);
	}

	CHECK(unlink(path));
}
END_TEST()

FN_TEST(xattr_remove_then_get_enodata)
{
	const char *path = BASE_DIR "/removefile";
	int fd = CHECK(open(path, O_CREAT | O_WRONLY, 0644));
	CHECK(close(fd));

	TEST_SUCC(setxattr(path, "user.gone", "tmp", 3, 0));
	TEST_SUCC(removexattr(path, "user.gone"));

	char buf[64] = { 0 };
	TEST_ERRNO(getxattr(path, "user.gone", buf, sizeof(buf)), ENODATA);

	CHECK(unlink(path));
}
END_TEST()

FN_TEST(xattr_get_nonexistent_enodata)
{
	const char *path = BASE_DIR "/noattr";
	int fd = CHECK(open(path, O_CREAT | O_WRONLY, 0644));
	CHECK(close(fd));

	char buf[64] = { 0 };
	TEST_ERRNO(getxattr(path, "user.nonexistent", buf, sizeof(buf)),
		   ENODATA);

	CHECK(unlink(path));
}
END_TEST()
